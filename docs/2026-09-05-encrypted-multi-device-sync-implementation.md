...
## 2026-09-06 플로팅 동기화 버튼 숨김

- 화면 양쪽에 플로팅 버튼이 동시에 표시되지 않도록 동기화 상태 버튼을 완전히 숨겼다.
- 자동 동기화 listener와 offline queue 재전송은 그대로 유지한다.
- 마지막 동기화 시간, 기기 상태 및 `지금 동기화`는 `설정 → 내 기기` 화면에서 확인·실행한다.
