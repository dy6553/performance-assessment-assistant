...
2026-09-17 운영 배포에서 공통 Registry 동기화를 1회 실행해 `approved=10`, `revoked=2`를 확인했다. 이후 수행도우미 Supabase를 다시 조회한 결과 live 승인 모델 10개가 시험온 중앙 API가 반환하는 10개와 동일하게 맞춰졌다. 초기 동기화용 일회성 빌드 스크립트는 실행 후 제거했고 `npm run build`는 다시 일반 `next build`로 복구했다.
