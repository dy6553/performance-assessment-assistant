...
## 2026-09-17 즉시 심사 실행 요청 점검

사용자 요청에 따라 운영 심사를 즉시 실행하는 경로를 점검했다. 수행도우미의 `/api/internal/model-catalog/sync`는 `CRON_SECRET` Bearer 인증을 요구한다.

- Vercel 프로덕션 런타임에는 보호용 `CRON_SECRET`이 존재하지만 빌드 환경에는 노출되지 않아 빌드 단계에서 호출할 수 없었다.
- GitHub Actions Secret에도 `CRON_SECRET`이 등록되어 있지 않아 보호된 운영 심사 API를 Actions에서 호출하는 방식도 사용할 수 없었다.
- 인증 우회나 공개 임시 엔드포인트는 추가하지 않았다.
- 두 차례의 실행 확인용 임시 파일/워크플로와 빌드 변경은 모두 제거했고 정상 빌드 명령을 복구했다.
- 운영 DB에는 이번 요청으로 임의 승인/거절 값을 직접 쓰지 않았다.

확인 시점 기준 수행도우미 DB에서 정책 버전 `2026-09-17.3-priority-publisher-korean`으로 새로 완료된 심사 행은 0개다. 즉 시험온과 달리 이번 요청에서 수행도우미의 보호된 런타임 심사는 실제 실행되지 않았다. 기존 SUPER_ADMIN 수동 갱신 경로와 `CRON_SECRET` 기반 내부 동기화 경로는 그대로 유지한다.
