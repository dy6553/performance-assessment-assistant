...
} from "@/features/assessment/stage-flow";
import {
...
import {
  ...
  type ResearchResult,
} from "@/features/assessment/stage-schemas";
import { readApiResponse } from "@/lib/http/client-response";

import { Icon } from "./icons";

type EditableTarget = "assignment" | "analysis" | "research" | "plan" | "draft" | "none";
type ChatMessage = { role: "user" | "assistant"; content: string };
type AiChange = { id: string; path: string; title: string; description: string; value: unknown };
type AssistantResult = { reply: string; target: EditableTarget; changes: AiChange[]; proposedValue: unknown | null };

const chatStorageKey = "assessment-ai-chat-v2";
...
export function EnhancedAiAssistant() {
  ...
  async function submit() {
    ...
    const stage = describeStage(pathname, context);
    ...
    try {
      const response = await fetch("/api/assignment/stage-assistant", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pathname, stage, userMessage: message, messages: recent, ...context }),
      });
  ...
  const context = typeof window === "undefined" ? null : readContext(pathname);
  const stage = context ? describeStage(pathname, context) : "수행평가 준비";
          ...
          <section aria-label="AI 도우미" aria-modal="true" className="absolute inset-x-0 bottom-0 flex max-h-[88dvh] flex-col rounded-t-[2rem] border border-violet-100 bg-white shadow-2xl md:inset-y-4 md:left-auto md:right-4 md:w-[30rem] md:rounded-[2rem]" role="dialog">
            <header className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-xs font-semibold text-slate-500">현재 단계의 전문 프롬프트와 이전 결과를 함께 적용합니다.</p>
              </div>
              <button aria-label="닫기" className="grid size-10 shrink-0 place-items-center rounded-full bg-slate-100 text-slate-600" onClick={() => setOpen(false)} type="button"><Icon className="size-5" name="close" /></button>
            </header>

            <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4 sm:px-5">
              {!messages.length ? (
                <div className="rounded-2xl border border-violet-100 bg-violet-50 p-4">
                  <p className="text-sm font-black text-violet-950">이 단계에서 무엇을 점검할까요?</p>
                  <p className="mt-1 text-sm leading-6 text-violet-700">주제선정·자료검증·수행설계·초안·최종검증 중 현재 단계에 맞는 기준으로 답합니다.</p>
                </div>
              ) : null}
              ...
              {loading ? <p className="rounded-2xl bg-violet-50 px-4 py-3 text-sm font-black text-violet-700">현재 단계 전문 기준으로 확인하는 중입니다...</p> : null}
                  ...
                  <div className="mt-3 space-y-2">
                    {result.changes.map((change) => (
                      <label className="flex cursor-pointer items-start gap-3 rounded-xl bg-white p-3" key={change.id}>
                        <input checked={selected.includes(change.id)} className="mt-1 size-4" onChange={(event) => setSelected((current) => event.target.checked ? Array.from(new Set([...current, change.id])) : current.filter((id) => id !== change.id))} type="checkbox" />
                        <span><span className="block text-sm font-black text-slate-900">{change.title}</span><span className="mt-1 block text-xs font-semibold leading-5 text-slate-500">{change.description}</span></span>
                      </label>
                    ))}
                  </div>
...
function readContext(pathname: string): {
  ...
  const research = readSession<ResearchResult>(assessmentResearchStorageKey);
  const plan = readSession<ExecutionPlanResult>(assessmentExecutionPlanStorageKey);
  ...
  let target: EditableTarget = "none";
  if (draft) target = "draft";
  else if (plan) target = "plan";
  else if (research) target = "research";
  else if (analysis) target = "analysis";
  else if (assignment || pathname.startsWith("/assignment")) target = "assignment";
  return { assignment, analysis, research, plan, draft, verification, target };
}

function describeStage(pathname: string, context: ReturnType<typeof readContext>) {
  ...
  if (pathname.includes("/workspace")) {
    if (!context.research) return "자료조사·출처검증";
    if (!context.plan) return "수행설계·목차";
    if (!context.draft) return "초안 작성";
    if (context.verification) return "최종 검증 후 수정";
    return "초안 수정·점검";
  }
...
function writeSession(key: string, value: unknown) { try { window.sessionStorage.setItem(key, JSON.stringify(value)); } catch { /* Keep chat usable in memory. */ } }
function removeSession(key: string) { try { window.sessionStorage.removeItem(key); } catch { /* Ignore restricted storage. */ } }
