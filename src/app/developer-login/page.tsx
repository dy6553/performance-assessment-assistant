  ...
  return (
    <main className="mx-auto min-h-[calc(100dvh-4rem)] max-w-xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-7 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <p className="text-sm font-black text-violet-700">GPT 관리자 전용</p>
        <h1 className="mt-2 text-3xl font-black tracking-[-0.04em] text-slate-950">GPT 관리자 로그인</h1>
        <p className="mt-3 text-sm font-semibold leading-7 text-slate-600">이메일 없이 개발자 ID와 비밀번호로 로그인합니다. gpt-admin 계정은 최고 관리자 화면으로 바로 이동합니다.</p>

        <form action={action} className="mt-6 space-y-4">
          <label className="block">
            <span className="mb-2 block text-sm font-black text-slate-700">개발자 ID</span>
            <input autoComplete="username" className="min-h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 font-semibold" name="developerId" pattern="[A-Za-z0-9_-]{3,32}" placeholder="gpt-admin" required />
          </label>
          <label className="block">
          ...
          <button className="min-h-12 w-full rounded-2xl bg-violet-600 px-5 font-black text-white disabled:opacity-60" disabled={pending} type="submit">{pending ? "확인 중…" : "GPT 관리자 로그인"}</button>
        </form>
      </div>
    </main>
  );
