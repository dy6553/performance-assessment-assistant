...
export function publicApiError(error: unknown, fallbackMessage: string) {
  ...
  if (/NVIDIA_(404|410)|사용 가능한 승인 모델이 없습니다/i.test(message)) {
    return "현재 사용 가능한 AI 모델을 전환하고 있습니다. 잠시 후 다시 시도해 주세요.";
  }
  if (/6페이지 이하/.test(message)) return message;
  if (/password|encrypted/i.test(message)) return "암호가 없는 PDF로 다시 올려 주세요.";
  if (/NVIDIA(?:_API_KEY|_\d{3})|SUPABASE|production 승인|model_registry/i.test(message)) {
